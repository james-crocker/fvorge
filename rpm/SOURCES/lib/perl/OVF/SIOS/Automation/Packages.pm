...
package OVF::SIOS::Automation::Packages;
...
sub apply ( \% ) {
	...
	my $property        = 'sios.automation.setup';
...
sub install ( \% ) {
	...
	my %automateVars = %{ $OVF::SIOS::Automation::Vars::automate{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

	OVF::Manage::Packages::install( %options, @{ $automateVars{$product}{packages} } );
...
sub remove ( \% ) {
	...
	OVF::Manage::Packages::remove( %options, @{ $automateVars{$product}{packages} } );
