...
package OVF::Service::Security::SSH::Packages;
...
sub apply ( \% ) {
    ...
    my $property = 'service.security.ssh.packages';

    if ( !defined $OVF::Service::Security::SSH::Vars::ssh{$distro}{$major}{$minor}{$arch} ) {
...
sub install ( \% ) {
    ...
    my %sshVars = %{ $OVF::Service::Security::SSH::Vars::ssh{$distro}{$major}{$minor}{$arch} };

    Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

    OVF::Manage::Packages::install( %options, @{ $sshVars{packages} } );
...
sub remove ( \% ) {
    ...
    OVF::Manage::Packages::remove( %options, @{ $sshVars{packages} } );
