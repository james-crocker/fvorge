...
package OVF::Service::Database::Sybase::Module;
...
use OVF::Service::Database::Sybase::Vars;
use OVF::State;
use OVF::Vars::Common;
use SIOS::CommonVars;
...
sub apply ( \% ) {
	...
	my $property = 'service.database.sybase.enabled';

	if ( !defined $OVF::Service::Database::Sybase::Vars::sybase{$distro}{$major}{$minor}{$arch} ) {
...
sub create ( \% ) {
	...
	my %sybaseVars = %{ $OVF::Service::Database::Sybase::Vars::sybase{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ...} );

	# CREATE Groups FIRST as USER creation may rely on them existing first.
	OVF::Manage::Groups::create( %options, %{ $sybaseVars{groups} } );
	OVF::Manage::Users::create( %options, %{ $sybaseVars{users} } );

	OVF::Manage::Directories::create( %options, %{ $sybaseVars{directories} } );
	OVF::Manage::Files::create( %options, %{ $sybaseVars{files} } );
...
sub destroy ( \% ) {
	...
	OVF::Manage::Directories::destroy( %options, %{ $sybaseVars{directories} } );
	OVF::Manage::Files::destroy( %options, %{ $sybaseVars{files} } );

	OVF::Manage::Users::destroy( %options, %{ $sybaseVars{users} } );
	OVF::Manage::Groups::destroy( %options, %{ $sybaseVars{groups} } );
