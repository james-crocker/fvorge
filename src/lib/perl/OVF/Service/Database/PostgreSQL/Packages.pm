...
package OVF::Service::Database::PostgreSQL::Packages;
...
sub apply ( \% ) {
	...
	my $property = 'service.database.postgresql.packages';
...
sub install ( \% ) {
	...
	my %pgsqlVars = %{ $OVF::Service::Database::PostgreSQL::Vars::pgsql{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

	OVF::Manage::Packages::install( %options, @{ $pgsqlVars{packages} } );
...
sub remove ( \% ) {
	...
	OVF::Manage::Packages::remove( %options, @{ $pgsqlVars{packages} } );
