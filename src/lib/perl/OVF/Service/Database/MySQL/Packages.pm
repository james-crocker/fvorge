...
package OVF::Service::Database::MySQL::Packages;
...
sub apply ( \% ) {
	...
	my $property = 'service.database.mysql.packages';
...
sub install ( \% ) {
	...
	my %mysqlVars = %{ $OVF::Service::Database::MySQL::Vars::mysql{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

	OVF::Manage::Packages::install( %options, @{ $mysqlVars{packages} } );
...
sub remove ( \% ) {
	...
	OVF::Manage::Packages::remove( %options, @{ $mysqlVars{packages} } );
