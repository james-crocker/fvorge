...
package OVF::Service::App::Samba::Packages;
...
sub apply ( \% ) {
	...
	my $property = 'service.app.samba.packages';
...
sub install ( \% ) {
	...
	my %sambaVars = %{ $OVF::Service::App::Samba::Vars::samba{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

	OVF::Manage::Packages::install( %options, @{ $sambaVars{packages} } );
...
sub remove ( \% ) {
	...
	OVF::Manage::Packages::remove( %options, @{ $sambaVars{packages} } );
