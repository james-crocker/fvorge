...
package OVF::Service::Security::SSHD::Packages;
...
sub apply ( \% ) {
    ...
    my $property = 'service.security.sshd.packages';

    if ( !defined $OVF::Service::Security::SSHD::Vars::sshd{$distro}{$major}{$minor}{$arch} ) {
...
sub install ( \% ) {
    ...
    OVF::Manage::Packages::install( %options, @{ $sshdVars{packages} } );
...
sub remove ( \% ) {
    ...
    OVF::Manage::Packages::remove( %options, @{ $sshdVars{packages} } );
