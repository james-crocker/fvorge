...
package OVF::Service::Graphic::XServer::Packages;
...
use OVF::Service::Graphic::XServer::Vars;
use OVF::State;
...
sub apply ( \% ) {
	...
	my $property = 'service.graphic.xserver.packages';

	if ( !defined $OVF::Service::Graphic::XServer::Vars::xserver{$distro}{$major}{$minor}{$arch} ) {
...
sub install ( \% ) {
	...
	my %xVars = %{ $OVF::Service::Graphic::XServer::Vars::xserver{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

	OVF::Manage::Packages::install( %options, @{ $xVars{packages} } );
...
sub remove ( \% ) {
	...
	OVF::Manage::Packages::remove( %options, @{ $xVars{packages} } );
