...
package OVF::Service::Security::SELINUX::Module;
...
use OVF::Service::Security::SELINUX::Vars;
use OVF::State;
...
sub apply ( \% ) {
	...
	my $property = 'service.security.selinux.enabled';

	if ( !defined $OVF::Service::Security::SELINUX::Vars::selinux{$distro}{$major}{$minor}{$arch} ) {
...
sub enable ( \% ) {
	...
	my %selinuxVars = %{ $OVF::Service::Security::SELINUX::Vars::selinux{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ...} );

	OVF::Manage::Files::create( %options, %{ $selinuxVars{enabled} } );
...
sub disable ( \% ) {
	...
	OVF::Manage::Files::create( %options, %{ $selinuxVars{disabled} } );
