...
package OVF::Service::Storage::MD::Packages;
...
sub apply ( \% ) {
	...
	my $property = 'service.md.packages';

	if ( !defined $OVF::MD::Vars::md{$distro}{$major}{$minor}{$arch} ) {
...
sub install ( \% ) {
	...
	my %mdVars = %{ $OVF::MD::Vars::md{$distro}{$major}{$minor}{$arch} };

	Sys::Syslog::syslog( 'info', qq{$action INITIATE ... } );

	OVF::Manage::Packages::install( %options, @{ $mdVars{packages} } );
...
sub remove ( \% ) {
	...
	OVF::Manage::Packages::remove( %options, @{ $mdVars{packages} } );
